...
async fn test_basic_date_arithmetic() {
    ...
    let stmt = parse_sql("SELECT CURRENT_DATE - EXTRACT(DAY FROM CURRENT_DATE) + 1 as month_start")
        .unwrap();
    match executor.execute(&stmt[0]).await {
        Ok(result) => {
    ...
    let stmt =
        parse_sql("SELECT CURRENT_DATE - EXTRACT(DAY FROM CURRENT_DATE) as days_back").unwrap();
    match executor.execute(&stmt[0]).await {
        Ok(result) => {
