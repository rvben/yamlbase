...
async fn test_advanced_join_types() {
    ...
    match executor.execute(stmt).await {
        Ok(result) => {
            println!(
                "   ✅ FULL OUTER JOIN works! Got {} rows",
                result.rows.len()
            );
    ...
    let stmts =
        parse_sql("SELECT e.name, d.dept_name FROM employees e CROSS JOIN departments d").unwrap();
    let stmt = &stmts[0];
    match executor.execute(stmt).await {
        Ok(result) => {
            println!("   ✅ CROSS JOIN works! Got {} rows", result.rows.len());
            println!(
                "     Expected: {} rows (3 employees × 2 departments = 6)",
                3 * 2
            );
        }
...
async fn test_complex_join_scenarios() {
    ...
    orders_table
        ...
        .unwrap();
    orders_table
        .insert_row(vec![Value::Integer(102), Value::Null, Value::Double(25.0)])
        .unwrap(); // Anonymous order
    ...
    match executor.execute(stmt).await {
        Ok(result) => {
            println!(
                "   ✅ Complex FULL OUTER JOIN works! Got {} rows",
                result.rows.len()
            );
            for (i, row) in result.rows.iter().enumerate() {
                println!("     Row {}: {:?}", i + 1, row);
