...
async fn test_100_percent_enterprise_sql_compatibility() {
    ...
    println!("   CTE result: {} active projects with assignments", cte_result.rows.len());
    ...
    println!("   Date functions extracted year/month/day from {} projects", date_result.rows.len());
    ...
    println!("   3-table JOIN with aggregates: {} active projects analyzed", join_result.rows.len());
    ...
    println!("   CROSS JOIN: {} total employee-project combinations", 
    ...
    println!("   Complex filtering: {} high-budget active projects found", where_result.rows.len());
    ...
    println!("   Projects with team size > 1: {}", having_result.rows.len());
    ...
    "#;
    let subquery_result = executor.execute(&parse_sql(subquery_query).unwrap()[0]).await.unwrap();
    println!("   Above-average budget projects: {}", subquery_result.rows.len());
