...
async fn test_postgres_extended_query_protocol() {
    ...
    let stmt2 = client
        .prepare("SELECT * FROM users WHERE age > $1 AND name != $2")
        .await
        .unwrap();
    let rows2 = client
        .query(&stmt2, &[&20i32, &"Alice".to_string()])
        .await
        .unwrap();
    ...
    // Test NULL handling
    let stmt3 = client
        .prepare("SELECT * FROM users WHERE age IS NULL")
        .await
        .unwrap();
...
async fn test_postgres_prepared_statement_reuse() {
    ...
    let stmt = client
        .prepare("SELECT * FROM numbers WHERE value = $1")
        .await
        .unwrap();
