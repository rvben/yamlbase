...
fn test_mysql_system_variables() {
    ...
    let result = std::panic::catch_unwind(|| {
        ...
        test_query(
            ...
            vec!["8.0.35-yamlbase", "1"],
        );

        // Test system variables mixed with regular queries
        test_query(
            &mut stream,
            "SELECT @@version, 42, 'hello'",
            vec!["8.0.35-yamlbase", "42", "hello"],
        );
    });
...
fn connect_and_auth() -> TcpStream {
    let mut stream = TcpStream::connect("127.0.0.1:13308").expect("Failed to connect to server");
...
fn test_query(stream: &mut TcpStream, query: &str, expected_values: Vec<&str>) {
    ...
    for _ in 0..column_count {
        stream
            .read_exact(&mut header)
            .expect("Failed to read column header");
        let length = u32::from_le_bytes([header[0], header[1], header[2], 0]);
    ...
    for (i, (actual, expected)) in values.iter().zip(expected_values.iter()).enumerate() {
        assert_eq!(
            actual, expected,
            "Query '{}' value {} mismatch: got '{}', expected '{}'",
            query, i, actual, expected
        );
    }
}
...
fn test_error(stream: &mut TcpStream, query: &str, expected_error: &str) {
    ...
    assert_eq!(
        response[0], 0xff,
        "Expected error packet for query '{}'",
        query
    );
...
fn test_init_db(stream: &mut TcpStream, database: &str) {
    ...
    stream
        .read_exact(&mut header)
        .expect("Failed to read init_db response header");
    let length = u32::from_le_bytes([header[0], header[1], header[2], 0]);
    let mut response = vec![0u8; length as usize];
    stream
        .read_exact(&mut response)
        .expect("Failed to read init_db response");
...
fn test_unknown_command(stream: &mut TcpStream, command: u8) {
    ...
    stream
        .write_all(&packet)
        .expect("Failed to send unknown command");
    ...
    assert_eq!(
        response[0], 0xff,
        "Unknown command should return error packet"
    );
    let error_msg = String::from_utf8_lossy(&response[9..]);
    assert!(
        error_msg.contains("Unknown command"),
        "Error message '{}' should contain 'Unknown command'",
        error_msg
    );
}
