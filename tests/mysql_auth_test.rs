...
fn test_mysql_auth_success() {
    ...
    let result = std::panic::catch_unwind(|| {
        let mut stream = TcpStream::connect("127.0.0.1:13307")
            .expect("Failed to connect to server");
        
        let (auth_data, sequence) = read_handshake(&mut stream);
        
        // Send correct credentials
        send_auth_response(&mut stream, sequence, "dbadmin", "securepass123", &auth_data);
        
        let auth_result = read_auth_result(&mut stream);
        assert_eq!(auth_result, 0x00, "Authentication should succeed with correct credentials");
    });
...
fn test_mysql_auth_wrong_username() {
    ...
    let result = std::panic::catch_unwind(|| {
        ...
        // Send wrong username
        send_auth_response(&mut stream, sequence, "wronguser", "securepass123", &auth_data);
        
        let auth_result = read_auth_result(&mut stream);
        assert_eq!(auth_result, 0xff, "Authentication should fail with wrong username");
    });
...
fn test_mysql_auth_wrong_password() {
    ...
    let result = std::panic::catch_unwind(|| {
        ...
        let auth_result = read_auth_result(&mut stream);
        assert_eq!(auth_result, 0xff, "Authentication should fail with wrong password");
    });
...
fn test_mysql_auth_empty_password() {
    ...
    let result = std::panic::catch_unwind(|| {
        ...
        let auth_result = read_auth_result(&mut stream);
        assert_eq!(auth_result, 0xff, "Authentication should fail with empty password");
    });
...
fn start_test_server() -> std::process::Child {
    std::process::Command::new("cargo")
        .args(&["run", "--", "-f", "examples/database_with_auth.yaml", "--protocol", "mysql", "-p", "13307"])
        .spawn()
...
fn send_auth_response(stream: &mut TcpStream, sequence: u8, username: &str, password: &str, auth_data: &[u8]) {
