...
async fn test_large_result_set_performance() {
    ...
    let mut large_table = Table {
        name: "large_dataset".to_string(),
        columns: vec![
            Column { name: "id".to_string(), sql_type: SqlType::Integer },
            Column { name: "name".to_string(), sql_type: SqlType::Text },
            Column { name: "email".to_string(), sql_type: SqlType::Text },
            Column { name: "age".to_string(), sql_type: SqlType::Integer },
            Column { name: "salary".to_string(), sql_type: SqlType::Double },
            Column { name: "department".to_string(), sql_type: SqlType::Text },
            Column { name: "created_at".to_string(), sql_type: SqlType::Timestamp },
            Column { name: "is_active".to_string(), sql_type: SqlType::Boolean },
        ],
    ...
    for i in 1..=10000 {
        ...
        large_table.data.push(vec![
            Value::Integer(i),
            Value::Text(format!("User_{:04}", i)),
            Value::Text(format!("user{}@company.com", i)),
            Value::Integer(25 + (i % 40)),
            Value::Double(50000.0 + (i as f64 * 100.0)),
            Value::Text(dept.to_string()),
            Value::Timestamp(chrono::NaiveDateTime::from_timestamp_opt(1640995200 + (i * 86400), 0).unwrap()),
            Value::Boolean(i % 3 != 0),
        ]);
    }
    ...
    let result = timeout(Duration::from_secs(5), executor.execute(&queries[0])).await
    ...
    println!("  ✓ Full scan completed in {:?} ({} rows/sec)", 
             duration, (10000.0 / duration.as_secs_f64()) as i32);
    
    // Test 2: Filtered query with WHERE clause (should be fast due to indexing)
    println!("  Test 2: Filtered query with WHERE clause...");
    let start = Instant::now();
    let queries = parse_sql("SELECT * FROM large_dataset WHERE department = 'Engineering'").unwrap();
    let result = timeout(Duration::from_secs(2), executor.execute(&queries[0])).await
        .expect("Filtered query should complete within 2 seconds")
        .expect("Query should succeed");
    let duration = start.elapsed();
    
    assert_eq!(result.rows.len(), 2000); // Every 5th record
    println!("  ✓ Filtered query completed in {:?} ({} matching rows)", duration, result.rows.len());
    
    // Test 3: Aggregation over large dataset
    println!("  Test 3: Aggregation query over large dataset...");
    let start = Instant::now();
    let queries = parse_sql("SELECT department, COUNT(*), AVG(salary) FROM large_dataset GROUP BY department").unwrap();
    let result = timeout(Duration::from_secs(3), executor.execute(&queries[0])).await
        .expect("Aggregation should complete within 3 seconds")
        .expect("Query should succeed");
    let duration = start.elapsed();
    
    assert_eq!(result.rows.len(), 5); // 5 departments
    println!("  ✓ Aggregation completed in {:?} ({} groups)", duration, result.rows.len());
    ...
    println!("  ✓ ORDER BY LIMIT completed in {:?} (top {} rows)", duration, result.rows.len());
...
async fn test_memory_efficient_processing() {
    ...
    let mut wide_table = Table {
        name: "wide_data".to_string(),
        columns: (0..50).map(|i| Column {
            name: format!("col_{}", i),
            sql_type: SqlType::Text,
        }).collect(),
        primary_key: Some("col_0".to_string()),
        data: vec![],
    };
    ...
    for i in 1..=5000 {
        let mut row = vec![];
        for j in 0..50 {
            row.push(Value::Text(format!("data_{}_{}", i, j)));
        }
        wide_table.data.push(row);
    }
    ...
    let result = timeout(Duration::from_secs(10), executor.execute(&queries[0])).await
    ...
    println!("  ✓ Wide result set processed in {:?} ({}x{} = {} cells)", 
             duration, result.rows.len(), result.columns.len(), result.rows.len() * result.columns.len());
...
async fn test_concurrent_large_queries() {
    ...
    let mut test_table = Table {
        name: "concurrent_data".to_string(),
        columns: vec![
            Column { name: "id".to_string(), sql_type: SqlType::Integer },
            Column { name: "data".to_string(), sql_type: SqlType::Text },
        ],
        primary_key: Some("id".to_string()),
        data: vec![],
    };
    ...
    for i in 1..=5 {
        ...
        let handle = tokio::spawn(async move {
            let query = format!("SELECT * FROM concurrent_data WHERE id > {} ORDER BY id", i * 1000);
            let queries = parse_sql(&query).unwrap();
            timeout(Duration::from_secs(10), executor_clone.execute(&queries[0])).await
    ...
    let duration = start.elapsed();
    println!("  ✓ All 5 concurrent queries completed in {:?} (total {} rows)", duration, total_rows);
    
    // Queries should complete in reasonable time even when concurrent
    assert!(duration < Duration::from_secs(15), "Concurrent queries took too long: {:?}", duration);
...
async fn test_complex_join_performance() {
    ...
    let mut users_table = Table {
        ...
        columns: vec![
            Column { name: "user_id".to_string(), sql_type: SqlType::Integer },
            Column { name: "username".to_string(), sql_type: SqlType::Text },
            Column { name: "department_id".to_string(), sql_type: SqlType::Integer },
        ],
    ...
    let mut departments_table = Table {
        name: "departments".to_string(),
        columns: vec![
            Column { name: "dept_id".to_string(), sql_type: SqlType::Integer },
            Column { name: "dept_name".to_string(), sql_type: SqlType::Text },
        ],
        primary_key: Some("dept_id".to_string()),
        data: vec![],
    };
    
    for i in 1..=10 {
        departments_table.data.push(vec![
            Value::Integer(i),
    ...
    let mut orders_table = Table {
        name: "orders".to_string(),
        columns: vec![
            Column { name: "order_id".to_string(), sql_type: SqlType::Integer },
            Column { name: "user_id".to_string(), sql_type: SqlType::Integer },
            Column { name: "amount".to_string(), sql_type: SqlType::Double },
        ],
    ...
    println!("  ✓ Complex JOIN completed in {:?} ({} result rows)", duration, result.rows.len());
    
    // Verify the results make sense
    for row in &result.rows {
        if let (Value::Text(_), Value::Integer(count), Value::Double(amount)) = (&row[0], &row[1], &row[2]) {
...
async fn test_pagination_performance() {
    ...
    let mut large_table = Table {
        name: "paginated_data".to_string(),
        columns: vec![
            Column { name: "id".to_string(), sql_type: SqlType::Integer },
            Column { name: "title".to_string(), sql_type: SqlType::Text },
            Column { name: "content".to_string(), sql_type: SqlType::Text },
            Column { name: "score".to_string(), sql_type: SqlType::Double },
        ],
        primary_key: Some("id".to_string()),
        data: vec![],
    };
    ...
    for i in 1..=8000 {
        large_table.data.push(vec![
            ...
            Value::Text(format!("Content for item {} with some additional text to make it realistic", i)),
    ...
    for page_size in page_sizes {
        ...
        println!("    ✓ First page ({} rows) completed in {:?}", page_size, duration);
        
        // Test middle page (using OFFSET simulation with WHERE)
        let start = Instant::now();
        let offset_id = page_size * 5; // Skip to middle pages
        let query = format!(
            "SELECT * FROM paginated_data WHERE id > {} ORDER BY id LIMIT {}", 
            offset_id, page_size
        );
        let queries = parse_sql(&query).unwrap();
        let result = timeout(Duration::from_secs(2), executor.execute(&queries[0])).await
        ...
        assert_eq!(result.rows.len(), page_size);
        println!("    ✓ Middle page ({} rows) completed in {:?}", page_size, duration);
    }
