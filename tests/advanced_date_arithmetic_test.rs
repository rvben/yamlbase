...
async fn test_advanced_date_arithmetic_functions() {
    ...
    let year_result = executor.execute(&parse_sql(dateadd_year_query).unwrap()[0]).await.unwrap();
    assert_eq!(year_result.rows.len(), 1);
    
    // Test DATEADD month  
    let dateadd_month_query = "SELECT DATEADD('month', 3, '2024-01-15') as result";
    let month_result = executor.execute(&parse_sql(dateadd_month_query).unwrap()[0]).await.unwrap();
    assert_eq!(month_result.rows.len(), 1);
    
    // Test DATEADD day
    let dateadd_day_query = "SELECT DATEADD('day', 30, '2024-01-15') as result";
    let day_result = executor.execute(&parse_sql(dateadd_day_query).unwrap()[0]).await.unwrap();
    assert_eq!(day_result.rows.len(), 1);
    
    // Test DATEADD week
    let dateadd_week_query = "SELECT DATEADD('week', 2, '2024-01-15') as result";
    let week_result = executor.execute(&parse_sql(dateadd_week_query).unwrap()[0]).await.unwrap();
    ...
    let datediff_day_result = executor.execute(&parse_sql(datediff_day_query).unwrap()[0]).await.unwrap();
    assert_eq!(datediff_day_result.rows.len(), 1);
    
    let datediff_month_query = "SELECT DATEDIFF('month', '2024-01-15', '2024-04-15') as result";
    let datediff_month_result = executor.execute(&parse_sql(datediff_month_query).unwrap()[0]).await.unwrap();
    ...
    let date_add_day_result = executor.execute(&parse_sql(date_add_day_query).unwrap()[0]).await.unwrap();
    assert_eq!(date_add_day_result.rows.len(), 1);
    
    let date_add_month_query = "SELECT DATE_ADD('2024-01-15', 2, 'MONTH') as result";
    let date_add_month_result = executor.execute(&parse_sql(date_add_month_query).unwrap()[0]).await.unwrap();
    ...
    let date_sub_day_result = executor.execute(&parse_sql(date_sub_day_query).unwrap()[0]).await.unwrap();
    assert_eq!(date_sub_day_result.rows.len(), 1);
    
    let date_sub_month_query = "SELECT DATE_SUB('2024-05-31', 1, 'MONTH') as result";
    let date_sub_month_result = executor.execute(&parse_sql(date_sub_month_query).unwrap()[0]).await.unwrap();
    ...
    let nested_result = executor.execute(&parse_sql(nested_query).unwrap()[0]).await.unwrap();
    ...
    let neg_dateadd_result = executor.execute(&parse_sql(neg_dateadd_query).unwrap()[0]).await.unwrap();
    ...
    let neg_date_add_result = executor.execute(&parse_sql(neg_date_add_query).unwrap()[0]).await.unwrap();
    assert_eq!(neg_date_add_result.rows.len(), 1);
    
    // Test year boundary
    let year_boundary_query = "SELECT DATEADD('month', 11, '2024-02-01') as result";
    let year_boundary_result = executor.execute(&parse_sql(year_boundary_query).unwrap()[0]).await.unwrap();
...
async fn test_interval_arithmetic_patterns() {
    ...
    let interval_result = executor.execute(&parse_sql(interval_patterns_query).unwrap()[0]).await.unwrap();
