...
fn test_mysql_connection_and_simple_query() {
    ...
    let mut server = std::process::Command::new("cargo")
        .args(&["run", "--", "-f", "examples/database_with_auth.yaml", "--protocol", "mysql", "-p", "13306"])
        .spawn()
    ...
    let result = std::panic::catch_unwind(|| {
        // Connect to server
        let mut stream = TcpStream::connect("127.0.0.1:13306")
        ...
        // Read query response
        stream.read_exact(&mut header).expect("Failed to read query response header");
        let length = u32::from_le_bytes([header[0], header[1], header[2], 0]);
        let mut response = vec![0u8; length as usize];
        stream.read_exact(&mut response).expect("Failed to read query response");
        
        // Should get column count (1)
        assert_eq!(response[0], 1, "Should get 1 column");
        
        // Read column definition
        stream.read_exact(&mut header).expect("Failed to read column def header");
        let length = u32::from_le_bytes([header[0], header[1], header[2], 0]);
