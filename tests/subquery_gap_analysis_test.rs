...
async fn test_subquery_gap_analysis() {
    ...
    test_table
        .insert_row(vec![Value::Integer(1), Value::Integer(10)])
        .unwrap();
    test_table
        .insert_row(vec![Value::Integer(2), Value::Integer(20)])
        .unwrap();
    test_table
        .insert_row(vec![Value::Integer(3), Value::Integer(30)])
        .unwrap();
    ...
    let stmts =
        parse_sql("SELECT * FROM test WHERE EXISTS (SELECT 1 FROM test WHERE id = 1)").unwrap();
    let stmt = &stmts[0];
    ...
    let stmts =
        parse_sql("SELECT * FROM test WHERE id IN (SELECT id FROM test WHERE value > 15)").unwrap();
    let stmt = &stmts[0];
    match executor.execute(stmt).await {
    ...
    let stmts =
        parse_sql("SELECT id, (SELECT MAX(value) FROM test) as max_val FROM test WHERE id = 1")
            .unwrap();
    let stmt = &stmts[0];
    match executor.execute(stmt).await {
        Ok(result) => println!(
    ...
    let stmts =
        parse_sql("SELECT * FROM test WHERE value > (SELECT AVG(value) FROM test)").unwrap();
    let stmt = &stmts[0];
    match executor.execute(stmt).await {
        Ok(result) => println!(
    ...
    let stmts =
        parse_sql("SELECT * FROM test WHERE NOT EXISTS (SELECT 1 FROM test WHERE id = 999)")
            .unwrap();
    let stmt = &stmts[0];
    match executor.execute(stmt).await {
