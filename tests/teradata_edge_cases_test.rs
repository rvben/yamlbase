...
mod limit_clause_edge_cases {
    ...
    async fn test_limit_zero() {
        ...
        let statements =
            parse_sql_with_dialect("SELECT * FROM test_table LIMIT 0", SqlDialect::Teradata)
                .unwrap();
    ...
    async fn test_limit_negative_values() {
        let statements =
            parse_sql_with_dialect("SELECT * FROM test_table LIMIT -1", SqlDialect::Teradata)
                .unwrap();
        let executor = create_test_executor().await;
    ...
    async fn test_limit_very_large_values() {
        let statements = parse_sql_with_dialect(
            "SELECT * FROM test_table LIMIT 2000000000",
            SqlDialect::Teradata,
        )
        .unwrap();
        let executor = create_test_executor().await;
    ...
    async fn test_limit_string_overflow() {
        let statements = parse_sql_with_dialect(
            "SELECT * FROM test_table LIMIT 999999999999999999999",
            SqlDialect::Teradata,
        )
        .unwrap();
        let executor = create_test_executor().await;
    ...
    async fn test_limit_boundary_values() {
        ...
        let statements = parse_sql_with_dialect(
            "SELECT * FROM test_table LIMIT 1000000000",
            SqlDialect::Teradata,
        )
        .unwrap();
        let result = executor.execute(&statements[0]).await;
        ...
        let statements = parse_sql_with_dialect(
            "SELECT * FROM test_table LIMIT 999999999",
            SqlDialect::Teradata,
        )
        .unwrap();
        let result = executor.execute(&statements[0]).await;
...
mod string_concatenation_edge_cases {
    ...
    async fn test_concat_null_values() {
        let executor = create_test_executor().await;
        let statements = parse_sql_with_dialect(
            "SELECT name || large_text FROM test_table WHERE id = 2",
            SqlDialect::Teradata,
        )
        .unwrap();
        let result = executor.execute(&statements[0]).await.unwrap();
    ...
    async fn test_concat_memory_limits() {
        ...
        let statements = parse_sql_with_dialect(
            "SELECT large_text || large_text FROM test_table WHERE id = 3",
            SqlDialect::Teradata,
        )
        .unwrap();
        let result = executor.execute(&statements[0]).await.unwrap();
    ...
    async fn test_concat_incompatible_types() {
        ...
        let statements = parse_sql_with_dialect(
            "SELECT date_col || name FROM test_table WHERE id = 1",
            SqlDialect::Teradata,
        )
        .unwrap();
        let result = executor.execute(&statements[0]).await;
...
mod date_function_edge_cases {
    ...
    async fn test_last_day_leap_year() {
        ...
        if let Value::Date(date_val) = &result.rows[0][0] {
            assert_eq!(date_val.format("%Y-%m-%d").to_string(), "2024-02-29");
        } else {
...
mod dialect_parsing_edge_cases {
    ...
    fn test_teradata_vs_postgresql_dialect() {
        ...
        let teradata_result =
            parse_sql_with_dialect("SELECT name || id FROM test_table", SqlDialect::Teradata);
        let postgresql_result =
            parse_sql_with_dialect("SELECT name || id FROM test_table", SqlDialect::PostgreSQL);
    ...
    fn test_date_literal_parsing() {
        ...
        let result = parse_sql_with_dialect(
            "SELECT * FROM test_table WHERE date_col >= DATE '2024-01-01'",
            SqlDialect::Teradata,
        );
        assert!(result.is_ok());
...
mod performance_edge_cases {
    ...
    async fn test_memory_efficient_query() {
        ...
        let statements =
            parse_sql_with_dialect("SELECT COUNT(*) FROM test_table", SqlDialect::Teradata)
                .unwrap();
        let result = executor.execute(&statements[0]).await;
        assert!(result.is_ok());
    }
    ...
    async fn test_memory_efficient_limit() {
        ...
        let statements =
            parse_sql_with_dialect("SELECT * FROM test_table LIMIT 1000", SqlDialect::Teradata)
                .unwrap();
        let result = executor.execute(&statements[0]).await.unwrap();
...
mod error_handling_edge_cases {
    ...
    async fn test_table_not_found_error() {
        let executor = create_test_executor().await;
        let statements =
            parse_sql_with_dialect("SELECT * FROM nonexistent_table", SqlDialect::Teradata)
                .unwrap();
        let result = executor.execute(&statements[0]).await;
        assert!(result.is_err());
