...
impl TeradataProtocol {
    pub async fn new(config: Arc<Config>, storage: Arc<Storage>) -> crate::Result<Self> {
        let auth = TeradataAuth::new(config.username.clone(), config.password.clone());

        // Create QueryExecutor once during initialization to avoid hanging on every query
            ...
            match Message::decode(&mut buffer) {
                Ok(Some(message)) => match self.handle_message(message).await {
                    ...
                    Err(e) => {
                        ...
                        let error_response = Message::single(Parcel::error_parcel(
                            5001,
                            &format!("Message handling error: {}", e),
                        ));
                        stream
            ...
            match parcel.kind {
                ...
                ParcelKind::RunRequest => {
                    if !self.authenticated {
                        return Ok(Message::single(Parcel::error_parcel(
                            4001,
                            "Not authenticated",
                        )));
                    }
                ...
                _ => {
                    ...
                    response_parcels.push(Parcel::error_parcel(
                        6000,
                        &format!("Unsupported parcel type: {:?}", parcel.kind),
                    ));
                }
            }
        }
    ...
    async fn execute_translated_query(&self, sql: &str) -> anyhow::Result<Vec<Parcel>> {
        ...
        let statements = parse_sql_with_dialect(sql, SqlDialect::Teradata)
        ...
        // Execute the query using the pre-initialized QueryExecutor
        match self.executor.execute(statement).await {
            Ok(result) => {
                // Send data info parcel with column metadata
                let column_info: Vec<(String, String)> = result
                    .columns
                    .iter()
