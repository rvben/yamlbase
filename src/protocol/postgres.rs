...
impl PostgresProtocol {
    ...
    pub async fn handle_connection(&self, mut stream: TcpStream) -> crate::Result<()> {
        ...
        self.read_startup_message(&mut stream, &mut buffer, &mut state).await?;
        ...
        loop {
            ...
            match msg_type {
                ...
                _ => {
                    ...
                    self.send_error(&mut stream, "XX000", "Unsupported operation").await?;
                }
            }
        }
    ...
    async fn read_startup_message(
        ...
        if buffer.len() < 8 {
            return Err(YamlBaseError::Protocol("Invalid startup packet".to_string()));
        }
        ...
        if buffer.len() >= 5 && buffer[0] == b'p' {
            ...
            // Verify credentials
            if state.username.as_deref() == Some(&self.config.username) && password == self.config.password {
                state.authenticated = true;
                self.send_auth_ok(stream, state).await?;
            } else {
                self.send_error(stream, "28P01", "Authentication failed").await?;
                return Err(YamlBaseError::Protocol("Authentication failed".to_string()));
            }
        } else {
            return Err(YamlBaseError::Protocol("Expected password message".to_string()));
        }
    ...
    async fn send_auth_ok(&self, stream: &mut TcpStream, _state: &ConnectionState) -> crate::Result<()> {
        // Authentication OK
        let mut buf = BytesMut::new();
        buf.put_u8(b'R');
        buf.put_u32(8);
        ...
        self.send_parameter_status(stream, "server_version", "14.0").await?;
        ...
        self.send_parameter_status(stream, "TimeZone", "UTC").await?;
    ...
    async fn send_parameter_status(&self, stream: &mut TcpStream, name: &str, value: &str) -> crate::Result<()> {
        let mut buf = BytesMut::new();
        buf.put_u8(b'S');
    ...
    async fn handle_query(&self, stream: &mut TcpStream, query: &str) -> crate::Result<()> {
        ...
        let statements = match parse_sql(query) {
            Ok(stmts) => stmts,
            Err(e) => {
                self.send_error(stream, "42601", &format!("Syntax error: {}", e)).await?;
                self.send_ready_for_query(stream).await?;
                return Ok(());
            }
        };
    ...
    async fn send_query_result(&self, stream: &mut TcpStream, result: &crate::sql::executor::QueryResult) -> crate::Result<()> {
    ...
    async fn send_error(&self, stream: &mut TcpStream, code: &str, message: &str) -> crate::Result<()> {
        let mut buf = BytesMut::new();
        buf.put_u8(b'E');
