...
mod comprehensive_tests {
    ...
    async fn test_cte_union_all_support() {
        // Create test database with projects and allocations tables (similar to AAC)
        let mut db = Database::new("test_db".to_string());
    ...
    async fn test_multiple_ctes_with_complex_expressions() {
        // Create test database similar to the AAC scenario
        let mut db = Database::new("test_db".to_string());
    ...
    async fn test_cte_joins_with_aggregates() {
        ...
        // Test the exact Priority 1.3 query pattern from the AAC compatibility report
