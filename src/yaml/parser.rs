...
use crate::yaml::schema::{YamlColumn, YamlDatabase, SqlType};
...
pub async fn parse_yaml_database(path: &Path) -> crate::Result<Database> {
    ...
    info!("Successfully parsed database with {} tables", database.tables.len());
    Ok(database)
}
    ...
    match (yaml_value, sql_type) {
        ...
        (Value::Number(n), SqlType::Integer) => {
            if let Some(i) = n.as_i64() {
                ...
                Err(crate::YamlBaseError::TypeConversion(
        ...
        (Value::Number(n), SqlType::Double) => {
            if let Some(f) = n.as_f64() {
                ...
                Err(crate::YamlBaseError::TypeConversion(
                    format!("Cannot convert {:?} to double", n)
                ))
            }
        }
        ...
        (Value::Number(n), SqlType::Decimal(_, _)) => {
            ...
            match s.parse::<rust_decimal::Decimal>() {
                ...
                Err(_) => Err(crate::YamlBaseError::TypeConversion(
                    format!("Cannot convert {:?} to decimal", n)
                ))
            }
        }
        
        (Value::String(s), SqlType::Varchar(_) | SqlType::Text) => {
        ...
        (Value::String(s), SqlType::Uuid) => {
            match uuid::Uuid::parse_str(s) {
                Ok(u) => Ok(DbValue::Uuid(u)),
                Err(_) => Err(crate::YamlBaseError::TypeConversion(
                    format!("Cannot parse UUID: {}", s)
                ))
            }
        }
        ...
        _ => Err(crate::YamlBaseError::TypeConversion(
            format!("Cannot convert {:?} to {:?}", yaml_value, sql_type)
        ))
    }
}
    ...
    match default.to_uppercase().as_str() {
        ...
        _ => {
            ...
            let yaml_value: serde_yaml::Value = match sql_type {
                ...
                SqlType::Integer => serde_yaml::Value::Number(
                    serde_yaml::Number::from(default.parse::<i64>().map_err(|_| {
                        crate::YamlBaseError::TypeConversion(format!("Invalid integer: {}", default))
                    })?)
                ),
